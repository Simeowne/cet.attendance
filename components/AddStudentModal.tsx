...
interface AddStudentModalProps {
  onAddStudent: (student: Omit<Student, 'avatarUrl'>) => void;
  onClose: () => void;
}

const AddStudentModal: React.FC<AddStudentModalProps> = ({ onAddStudent, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!id.trim() || !name.trim() || !course.trim() || year === '' || !block.trim()) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg relative">
        ...
        <h2 className="text-2xl font-bold mb-4 text-center text-slate-800">Add New Student</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              id="studentId" type="text" value={id} onChange={(e) => setId(e.target.value)}
